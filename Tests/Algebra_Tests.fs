...
type ``Algebra Tests`` () = 
    ...
    [<Test>] 
    member v.``Test for functor equality.`` () = 
    ...
    [<Test>]
    member v.``Test of Algebra.subst.`` () =
    ...
    [<Test>]
    member v.``Test of match of variable and expression.`` () = 
        matchOK ("x", "S(Z())", "x->S(Z());")

    [<Test>]
    member v.``Test of match of constructor and variable.`` () = 
        matchNo ("Z()", "x")

    [<Test>]
    member v.``Test of match of constructor and constructor.`` () = 
    ...
    [<Test>]
    member v.``Test of Algebra.equiv.`` () =
